...
public class VectorObserver_Space_long_
  implements Vector_Space_long_,
             Changing,
             Dumpable,
             ManagedResource
{
    protected Vector_Space_long_ observed_vector;
    ...
    public VectorObserver_Space_long_ (CallContext context, Vector_Space_long_ observed_vector,VectorObserverEventPolicy event_policy, ModificationType modification_type) {
        ...
        if (this.observed_vector != null && modification_type != ModificationType.STATE_LOAD) {
            if (getEventPolicy(context) == VectorObserverEventPolicy.ON_DEMAND) {
                ...
                for (Space item : observed_vector.getIterable_Space_(context)) {
                    onAdd(context, item);
                }
            }
        }
    ...
    public VectorObserver_Space_long_ (CallContext context, Vector_Space_long_ observed_vector,VectorObserverEventPolicy event_policy) {
        this(context, observed_vector, event_policy, null);
    }

    public VectorObserver_Space_long_(CallContext context, Vector_Space_long_ observed_vector) {
        this(context, observed_vector, VectorObserverEventPolicy.ON_DEMAND);   
    }

    public void setObserved (CallContext context, Vector_Space_long_ observed_vector) {
        if (this.getObserved(context) != null) {
            for (Space item : this.observed_vector.getIterable_Space_(context)) {
    ...
    public Vector_Space_long_ getObserved (CallContext context) {
    ...
    protected void onAdd(CallContext context, Space item) {
    }

    protected void onRemove(CallContext context, Space item) {
    }
    ...
    public Space                                    set             (CallContext context, long index, Space item) {
        Space old_item = this.getObserved(context).set(context, index, item);
        if (old_item != null) { 
    ...
    public Space                                    replace         (CallContext context, long index, Space item) throws DoesNotExist {
        Space old_item = this.getObserved(context).replace(context, index, item);
        onRemove(context, old_item);
        ChangeEvent event = new RemoveEvent(context,old_item,index);   
        notify(context,event);
        onAdd(context, item);
    ...
    public Space                                    unset           (CallContext context, long index) {
        Space old_item = this.getObserved(context).unset(context, index);
        if (old_item != null) { 
           onRemove(context, old_item); 
           ChangeEvent event = new RemoveEvent(context,old_item,index);   
           notify(context,event);
    ...
    public Space                                    remove          (CallContext context, long index) throws DoesNotExist {
        Space old_item = this.getObserved(context).remove(context, index);
        onRemove(context, old_item);
        ChangeEvent event = new RemoveEvent(context,old_item,index);   
        notify(context,event);
        return old_item;
    ...
    public java.util.Iterator<Space> getIterator_Space_ (CallContext context) {
        return this.getObserved(context).getIterator_Space_(context);
    }
